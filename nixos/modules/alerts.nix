# Alerting: a handful of systemd timers that curl ntfy. This deliberately
# replaces any kind of event system / API / dashboard — the entire alerting
# stack is this file, and the point is that you can read all of it.
#
# Checks:
...
let
  # ── CHANGEME ── the one knob: your ntfy topic URL.
  # Treat the topic name as a secret-ish value (anyone who knows it can read
  # your alerts) — pick something unguessable, or point at a self-hosted ntfy.
  ntfyTopic = "https://ntfy.sh/CHANGEME-citadel-alerts";

  vmName = "work-vm";
  stateDir = "/run/citadel-alerts";

  # ntfy-send TITLE PRIORITY BODY — the single delivery path all checks use.
  # Retries cover blips; --max-time keeps a dead network from hanging a unit.
  ...
  smartdHook = pkgs.writeShellScript "smartd-ntfy" ''
    set -euo pipefail
    ${ntfySend} "citadel SMART: $SMARTD_FAILTYPE on $SMARTD_DEVICE" urgent \
      "$SMARTD_MESSAGE"
  '';
  ...
  diskCheck = check {
    name = "alert-disk";
    description = "ntfy alert: root filesystem >85% full";
    every = "1h";
    script = ''
      mkdir -p ${stateDir}
      ...
      if [ "$pct" -gt 85 ]; then
        if [ ! -f ${stateDir}/disk ]; then
          ${ntfySend} "citadel: disk $pct% full" high \
  ...
  vmCheck = check {
    name = "alert-vm-down";
    description = "ntfy alert: ${vmName} defined but not running >10min";
    every = "5min";
    script = ''
      mkdir -p ${stateDir}
      ...
      if [ $((now - since)) -ge 600 ] && [ ! -f ${stateDir}/vm-down-notified ]; then
        # priority "default", not "high": after a reboot this fires until you
        # unlock, which is expected. It's a reminder, not a page.
        ${ntfySend} "citadel: ${vmName} not running" default \
  ...
  tailscaleCheck = check {
    ...
    description = "ntfy alert: tailscaled not active";
    every = "5min";
    script = ''
      mkdir -p ${stateDir}
      ...
      if [ ! -f ${stateDir}/tailscale ]; then
        ${ntfySend} "citadel: tailscaled is DOWN" high \
  ...
  bootNotify = {
    alert-boot = {
      description = "ntfy alert: host booted";
      ...
      script = ''
        ${ntfySend} "citadel: host booted" default \
...
in
{
  ...
  # Wire smartd (enabled in monitoring.nix) to ntfy. "-a" = monitor
